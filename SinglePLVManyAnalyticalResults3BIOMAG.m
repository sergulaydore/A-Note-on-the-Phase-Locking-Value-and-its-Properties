...
lw = 2; fs = 15; ms = 8;
for idxB = [1 3]
%     plot(length2vec,meanPLVsampleUb(idxB,:),'b-d','LineWidth',lw,'MarkerSize',ms);
hold on;
 plot(length2vec,meanPLVsample(idxB,:),'r-s','LineWidth',lw);
%plot(length2vec,meanPLVvonmises(idxB,:),'r-*','LineWidth',lw,'MarkerSize',ms);
plot(length2vec,meanPLVcircgauss(idxB,:),'k-o','LineWidth',lw,'MarkerSize',ms);
...
%      '$\mbox{E}\left[{PLV}_{sample}\right]$',...
...
lw = 2; fs = 15;  ms = 8;
for idxB = [ 1 3]
%     plot(length2vec,log10(varPLVsampleUb(idxB,:)),'b-d','LineWidth',lw);
hold on;
 plot(length2vec,log10(varPLVsample(idxB,:)),'r-s','LineWidth',lw);
%plot(length2vec,log10(varPLVvonmises(idxB,:)),'r.-','LineWidth',lw);
plot(length2vec,log10(varPLVcircgauss(idxB,:)),'k-o','LineWidth',lw);
% plot(length2vec,meanPLVanalCircgauss(idxB,:),'m--','LineWidth',lw)
xlabel('Number of Observations','Fontsize',fs)
ylabel('Variance (log)','Fontsize',fs)
% h = legend('$\log_{10} \left(\mbox{Var} \left[ \sqrt{ {PLV}^2_{ub\_sample} } \right] \right)$',...
%     '$\log_{10} \left( \mbox{Var}\left[{PLV}_{sample}\right] \right)$',...
