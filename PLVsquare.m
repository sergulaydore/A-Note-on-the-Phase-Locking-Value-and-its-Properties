...
samplePLVSq(idxKappa,idxN) = (abs(mean(exp(sqrt(-1)*alpha))))^2;
samplePLVSqUb(idxKappa,idxN) = (n*samplePLVSq(idxKappa,idxN)-1)/(n-1);
    end
end
...
title('$|\hat{PLV}^2 -I_1({\kappa}) / I_0({\kappa})|$',...
...
title('$|\hat{PLV}^2_{ub} -I_1({\kappa}) / I_0({\kappa})|$',...
    'Interpreter','latex','Fontsize',fs)
xlabel('Number of Observations','Fontsize',fs)
axis([0 100 0 .35])
