...
for idxB = 1:length(bVec)
    ...
    A = [1 b; b 1];
    v = A*A';
     trueCorrCoeff(idxB) =  v(1,2)/sqrt(v(1,1)*v(2,2));
...
%     Ry2y2anal = convresult(101) ;
    ...
    Rz1z1anal = 2*Rx1x1anal;
    Rz2z2anal = 2*Rx2x2anal;
    Rz1z2anal = 2*Rx1x2anal;
    ...
    PLVcg(idxB) = pi/sqrt(2)/k12^2*abs(det(invKmat_anal))*(w^(3/2)*hypergeom([3/4,5/4],1,w^2)+ 3/4*w^(5/2)*hypergeom([5/4,7/4],2,w^2))
...
%         y(2,:) = filter_apply(x(2,:),h); %conv(yorig,h); %
        ...
        [invKmat_emp, r1_orig, r2_orig, rvec_orig, phase_mat_orig, z_orig] = f_amp_phase(x(1,:),x(2,:));
...
%             y(2,:) = filter_apply(x(2,:),h); %conv(yorig,h); %
        ...
        for lengthindex = 1:length(length2vec)
            ...
            x_part(1,:) = x(1,lengthall/10:lengthall/10+length2-1); %+ 0.02*rand(1,length2+1);
            x_part(2,:) = x(2,lengthall/10:lengthall/10+length2-1); %+ 0.02*rand(1,length2+1);
            
            % Circular Gaussian
            [invKmat_par, r1, r2, rvecorg, phase_mat, z] = f_amp_phase(x_part(1,:),x_part(2,:)); % parameters of the distribution in polar coordinates
...
lw = 3; fs = 15;
for idxB = 1:length(bVec)
    plot(length2vec,meanPLVsampleUb(idxB,:),'b--','LineWidth',lw);
hold on;
plot(length2vec,meanPLVsample(idxB,:),'r--','LineWidth',lw);
plot(length2vec,squeeze(PLVanalCircgauss(idxB,:,1)),'r','LineWidth',lw);
plot(length2vec,meanPLVcircgauss(idxB,:),'k--','LineWidth',lw);
% plot(length2vec,meanPLVanalCircgauss(idxB,:),'m--','LineWidth',lw)
xlabel('Number of Observations','Fontsize',fs)
legend('Sample','Sample UB','Analytical','Circ Gauss')
...
lw = 3; fs = 15;
for idxB = 1:length(bVec)
    plot(length2vec,log10(varPLVsampleUb(idxB,:)),'b','LineWidth',lw);
hold on;
plot(length2vec,log10(varPLVsample(idxB,:)),'r--','LineWidth',lw);
plot(length2vec,log10(varPLVvonmises(idxB,:)),'r','LineWidth',lw);
plot(length2vec,log10(varPLVcircgauss(idxB,:)),'k--','LineWidth',lw);
% plot(length2vec,meanPLVanalCircgauss(idxB,:),'m--','LineWidth',lw)
xlabel('Number of Observations','Fontsize',fs)
