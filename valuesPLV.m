...
dataDir =  ['F:\29.02.2012backup\Research Spring 2012\april\'...
        'paper_draft\matlab_codes\ResultsSignificance\lowBeta\PLV120\']; %\PPLVschelter
...
for idxExp =expVec
    ...
    for pIdx = 1:length(IndX)
        valuesGo(pIdx) = PLVgoActual(IndX(pIdx),IndY(pIdx));
        valuesNoGo(pIdx) = PLVnogoActual(IndX(pIdx),IndY(pIdx));
    end
...
dataDir =  ['F:\29.02.2012backup\Research Spring 2012\april\'...
        'paper_draft\matlab_codes\ResultsSignificance\lowBeta\PLV260\']; %\PPLVschelter
